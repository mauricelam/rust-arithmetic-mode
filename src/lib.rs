...
use quote::quote;
use syn::{Expr, ExprBinary, ExprLit, ExprUnary};
    ...
    fn recurse(expr: syn::Expr) -> anyhow::Result<TokenStream> {
        Ok(match expr {
            Expr::Binary(ExprBinary {
                ...
                let new_right = recurse(*right)?;
                checked_op(op)?
                    .map(|func| quote! { #new_left.zip(#new_right).and_then(|(l, r)| l.#func(r)) })
...
fn saturating_op(op: syn::BinOp) -> anyhow::Result<Option<TokenStream>> {
    Ok(match op {
        syn::BinOp::Add(_) => Some(quote! { saturating_add }),
        syn::BinOp::Sub(_) => Some(quote! { saturating_sub }),
        syn::BinOp::Mul(_) => Some(quote! { saturating_mul }),
        syn::BinOp::Div(_) => Some(quote! { saturating_div }),
        syn::BinOp::Rem(_) => Some(quote! { saturating_rem }),
        syn::BinOp::And(_)
        | syn::BinOp::Or(_)
        | syn::BinOp::BitXor(_)
        | syn::BinOp::BitAnd(_)
